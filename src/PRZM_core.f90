module PRZM_CORE
   ...
   use  constants_and_Variables, ONLY: is_temperature_simulated,soil_temp, nonequilibrium_flag,              &
    ...
    Kd_old,    kd_new, delx,   &
    thair_old, thair_new, &
    theta_zero, delt, number_subdelt, subdelt, Sorbed2, canopy_flow,   harvest_day , irrr,slp
...
!   real halftime !1/2 the time step for use initialization the predictor step
  ...
  ! CALL PLGROW(julday1900) !PLANT GROWTH
   ...
   ! call to write_outputfile moved to PRZM5 subroutine so iteration is available
