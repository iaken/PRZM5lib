...
SUBROUTINE INITL
  use constants_and_Variables, ONLY: min_evap_depth,                                    &
        ...
        runof, snow, foliar_degrade_loss, FOLPST, DCOFLX, SUPFLX, SDKFLX,   &  ! These require initialization
        Foliar_volatile_loss, potential_canopy_holdup, CINT, TDET  ! These require initialization
 ...
 ! integer  :: ISDAY,ISMON,ISTYR !INPUT from  weather file, start date
  ...
  integer :: start, xend
   ...
   theta_sat = 1.0 - bulkdensity/2.65
    ! removed from dll to prevent writing to file
...
!     TAPP... KG/HA  --->G/CM**2
     ...
     End Do
   ! initialize some variables that are set to zero in "constants_and_variables" to
  ! make sure they get reinitialized before each run
     runof                   = 0.0   
     snow                    = 0.0   !accumulated snow for each day
     foliar_degrade_loss     = 0.0   !foliar loss by degradation
     FOLPST                  = 0.0   !foliar storage
     DCOFLX                  = 0.0   !Pesticide outflow below soil core
     SUPFLX                  = 0.0 
     SDKFLX                  = 0.0 
     Foliar_volatile_loss    = 0.0    !volatilization
     potential_canopy_holdup = 0.0    !daily maximum water that the canopy can hold
     CINT             = 0.0    !the actual water held on the canopy during the current day
     TDET             = 0.0    !evapotranspiration in the topp ncom1 compartments    
