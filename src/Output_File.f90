Module Output_File
  ...
  SUBROUTINE write_outputfile(iteration, results)
    ...
    OUTPJJ,chem_id,NPLOTS, ARG,IARG2,PLNAME,MODE,DCOFLX,max_number_plots,Version_Number,julday1900
     
    use utilities
    
!   Outputs user specified time series to time series plotting files
    implicit none
    integer, intent(in) :: iteration  ! used a row number for final output to
    real*8, dimension(:,:), intent(inout) :: results
    integer :: start_compartment, end_compartment
    INTEGER  I,K
    real*8 ::  OUTPUT(NPLOTS)  ! all three, formerly max_number_plots
    REAL*8 ::  PNBRN(NPLOTS)
    real*8 ::  PRTBUF(NPLOTS)
...
!       Water fluxes (units of CM/DAY)
  ...
  !    IF (PLNAME(I) .EQ. 'RZFX') PNBRN(I)= RZFLUX(chem_id(i))  redundant
   ...
   results(iteration, :) = [real(current_year), real(current_month), real(current_day), (PRTBUF(I),I=1,NPLOTS) ]
